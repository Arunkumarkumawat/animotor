...
class RegionService
{
    public function getRegionByLatLng($lat, $lng){
        ...
        $firstRegionWithParentId = Region::whereContains('coordinates', new Point($lat, $lng, (int)env('POINT_SRID', 0)))
    ...
    public function getSpecialPlaceByLatLng($lat, $lng){
        return Region::whereContains('coordinates', new Point($lat, $lng, (int)env('POINT_SRID', 0)))
            ->where('is_active', true)
            ->latest()
