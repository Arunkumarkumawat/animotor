...
class Form extends Component
{
    ...
    public ?string $dynamic_pricing_adjustment_type = 'percentage_increase';
    public ?string $dynamic_pricing_adjustment_value;
    public ?string $dynamic_pricing_start_date;
...
//    TAX
    ...
    public ?string $calendarTab = 'availability';
    public function setCalendarTab($tab)
    {
    ...
    {
        $this->availabilities = $car->availabilities;
        $this->blackouts = $car->blackouts;     
        $this->regions = Region::withoutAirport()->select('id','name')->get();
...
//            'year' => ['required', 'string'],
    ...
    public function addExtras()
    {
        $this->validate([
            ...
            'extras.interval' => ['required'],
        ]);

        $newData = [
            'title' => $this->extras['title'],
            'price' => $this->extras['price'],
            'description' => $this->extras['description'],
            'interval' => $this->extras['interval'],
        ];
    ...
    {
        $this->dynamic_pricing_rule_name = '';
        $this->dynamic_pricing_adjustment_type = 'percentage_increase';
        $this->dynamic_pricing_adjustment_value = '';
        $this->dynamic_pricing_start_date = '';
        $this->dynamic_pricing_end_date = '';
    }

    public function removeDynamicPricing($index)
    {
    ...
    {
        $this->validate([
            'availability.day_of_week' => 'required',
            'availability.pickup_hours_start' => 'required',
            'availability.pickup_hours_end' => 'required',
            'availability.return_hours_start' => 'required',
            'availability.return_hours_end' => 'required',
        ]);

        $this->car->availabilities()->create($this->availability);
    ...
    {
        $this->car->availabilities()->where('id', $id)->update(['status' => $status]);
        $this->availabilities = $this->car->availabilities;
    }
    ...
    public function saveBlackout()
    {
        ...
        $this->car->blackouts()->create($this->blackout);
        $this->blackouts = $this->car->blackouts;
        $this->resetBlackout();
    }

    private function resetBlackout()
    {
        $this->blackout = [
            'start_date_time' => '',
            'end_date_time' => '',
